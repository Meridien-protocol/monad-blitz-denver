  ...
  return (
    <div>
      ...
      <div className="mb-3 flex gap-2">
        <button
          onClick={() => { setDirection("YES_FOR_NO"); setAmount(""); reset(); }}
        ...
        <button
          onClick={() => { setDirection("NO_FOR_YES"); setAmount(""); reset(); }}
          className={`flex-1 rounded py-2 text-sm font-bold transition-colors ${
      ...
      <div className="mb-3 flex gap-4 text-xs text-neutral-500">
        <span>
          YES: <span className="font-mono text-yes">{Number(formatEther(yesBalance)).toFixed(4)}</span>
        </span>
        <span>
          NO: <span className="font-mono text-no">{Number(formatEther(noBalance)).toFixed(4)}</span>
        </span>
      </div>
      ...
      {isSuccess && (
        <p className="mt-2 text-center text-xs text-yes">Swap confirmed.</p>
      )}
      {error && (
